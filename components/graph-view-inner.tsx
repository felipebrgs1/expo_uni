...
export default function GraphViewInner() {
  const { actualTheme } = useTheme();
  const isDark = actualTheme === 'dark';
  const { width: screenWidth, height: screenHeight } = useWindowDimensions();
  
  const data = useNoteStore((s) => s.graphData);
  ...
  useEffect(() => {
    if (dimensions.width === 0 || dimensions.height === 0 || !data?.nodes?.length) return;
    ...
    <View className="flex-1 overflow-hidden bg-background" onLayout={handleLayout}>
      {dimensions.width > 0 && dimensions.height > 0 && (
        <GestureDetector gesture={composed}>
          <Animated.View style={[containerStyle, { flex: 1, backgroundColor: 'rgba(0,0,0,0.01)' }]}>
